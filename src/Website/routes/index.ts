...
import {command} from "../../Discord/util/others/typing";
env.config();
...
router.get("/", function(req:Express.Request, res:Express.Response, next?:Express.NextFunction) {
  const commands = `${vars.commands.filter((command:command) => (command.folder !== "test" && command.folder !== "owner")).size} Text Commands`;
...
router.get("/commands", function(req:Express.Request, res:Express.Response, next?:Express.NextFunction) {
  ...
  if (!type) type = "text";
  res.render("index/commands", {type, search, commands: commands.filter((command:command) => (command.folder !== "test" && command.folder !== "owner")), slashes});
});
